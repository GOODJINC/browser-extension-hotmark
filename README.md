...
- 폴더 팝업에서 방향키, Home/End와 Enter로 탐색
- 폴더 기본 동작: 내용을 메뉴로 표시
...
`Ctrl+Alt` 조합은 AltGr 키와의 충돌을 막기 위해 Chromium에서 허용되지 않습니다. Windows 한국어 환경에서는 `Ctrl+Shift` 조합을 권장합니다. Chromium은 확장 프로그램 하나가 제안할 수 있는 기본 단축키를 최대 4개로 제한하므로 북마크 슬롯 1~4에만 `Ctrl+Shift+1`부터 `Ctrl+Shift+4`까지가 기본 제안되어 있습니다. 슬롯 5~10은 브라우저의 확장 단축키 화면에서 직접 지정해야 합니다.
